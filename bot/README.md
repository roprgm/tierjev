...
4. With the site deployed, `bun run bot:webhook register https://tierjev.com/api/webhooks/x` registers the webhook (X validates it with a CRC request right away) and subscribes the account to mentions; `bun run bot:webhook list` shows the result and `delete <webhookId>` removes one. The same can be done from the Webhooks tab of the app in the developer console.
